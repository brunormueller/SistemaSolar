...
const Funil = ({
  ...
  }, [isCreatedNewFunnel, funnelsBar, setIsCreatedNewFunnel]);
  ...
  const onSubmitStep = (data: any) => {
    setIsLoading(true);
    createNewStep(funil.id_kanban, data.title).then(() => {
  ...
  const deleteFunnel = (data: any) => {
    atualizarFunis((funis: any) => {
      ...
      return funis.filter((funnel: any) => funnel.id_kanban != funil.id_kanban);
  ...
  const [{ isOver }, dragFunnel] = useDrop({
    ...
    drop: (item: any) => {
      moveFunil(item.funilIndex, indexFunil, item.index, item.index, true);
    },
  });
  ...
  const moveFunnel = (position: number) => {
    atualizarFunis((prevFunnels: any) => {
      ...
      const proximoFunnel = newFunnels[Number(funil.index_kanban) + position];
      ...
      >
        ...
        >
          {steps.length > 0 && steps?.cards_kanban?.length > 0 ? (
            <>
              <div>
                Para deletar um funil é necessário que as etapas não contenham
                nenhum card nele!
              </div>
            ...
            <>
              <div>
                Tem certeza que deseja continuar com a deleção do funil?
              </div>
              {steps.length > 0 && (
